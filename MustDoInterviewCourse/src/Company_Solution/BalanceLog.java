...
public class BalanceLog {
	public double balancePoint(Log log) {
		...
		double totalWeight = log.weightUpto(length);

		for (double i = 1; i <= length; i = i + 1) {
			if (2 * log.weightUpto(i) == totalWeight) {
				return i;
			}

			if (log.weightUpto(i-1) == totalWeight-log.weightUpto(i)) {
				return i;
			}
		}
		return 0;
